...
const adminRegister = async (req, res) => {
 ...
 const adminLogin = async (req, res) => {
    try {
      ...
      jwt.sign({id: isAdmin._id}, process.env.SECRET, {expiresIn: '1d'}, async(err, token)=> {
          if (err){
             throw new Error(err)
          }
...
// Addmin create Jobs
 ...
 const adminCreateJobs = async (req, res) => {
    try{
     const {jobTitle, jobDescription, companyName, startDate, endDate} = req.body
     const newJob = jobsModel({
        jobTitle, jobDescription, companyName, startDate, endDate
     })
     const savedJob = await newJob.save()
     res.status(201).json({success: true, message: "Job Created Successfully", savedJob})
    }
    catch(err){
       console.error(err);
       res.status(404).json({
...
const getAllUsers = async (req, res) => {
    try {
        ...
        res.status(202).json({success: true, message: "View All Users Succesfully", allUsers})
    }
    catch(err) {
        console.error(err);
       res.status(404).json({
